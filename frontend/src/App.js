...
function FacialTab({ onResult }) {
  ...
  const analyzeFacial = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/api/predict/facial${explainParam}`, formData, { timeout: 400000 });
...
function CombinedTab({ onResult }) {
  ...
  const analyzeCombined = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/api/predict/combined${explainParam}`, formData, { timeout: 400000 });
      if (response.data.success) {
  ...
  const analyzeVideo = async () => {
    ...
    try {
      ...
      const explainParam = showExplainability ? '?explain=true' : '';
      const response = await axios.post(`${API_BASE}/api/predict/video${explainParam}`, formData, { timeout: 400000 });
      if (response.data.success) {
